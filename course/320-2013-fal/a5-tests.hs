module A5Tests where

import Interpreter
import Allocation

allTests = [
  show (failed evalTests),
  show (failed evalTests),
...
evalTests = [
  (1, eval ([], Value T `And` ((Value F `Or` Value T) `Or` Value T)), T),
  (2, eval ([("x", Number 2)], (Plus (Var "x") (Var "x"))), Number 4),
  (3, eval ([("x", Number 2)], Var "x" + Var "x" + Var "x"), Number 6),
  (4, eval ([("x", Number 2)], (Plus (Var "x") (Var "x"))), Number 4),
  (5, eval ([("a", T), ("b", F), ("c", F)], Not (Var "a")), F),
  (6, eval ([("a", T), ("b", Number 2)], Var "a" `Or` Not (Var "a")), T)
  ]

execTests = [
  (1, exec ([], End), []),
  (2, exec ([], Print (Value (Number 4)) End), [Number 4]),
  (3, exec ([], Print (Value (Number 4)) (Print (Value T) End)), [Number 4, T]),
  (4, exec ([("x", Number 2)], Print (4 + 4) (Print (Var "x") End)), [Number 8,Number 2]),
  (5, exec ([], Assign "x" (1 + 2) (Print (4 + 4) (Print (Var "x" + Var "x" + 2) End))), [Number 8,Number 8]),
  (6, exec ([], Assign "x" (1 + 2) (Assign "x" (Value T) (Print (Var "x") End))), [T]),
  (7, exec ([], Assign "a" (Value T `And` Value F) (Assign "x" 3 (Print (4 + 4) (Print (Var "x" + Var "x" + 2) (Print (Not (Var "a")) End))))), [Number 8,Number 8,T])
  ]
